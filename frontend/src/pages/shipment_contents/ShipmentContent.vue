<template>
  <div class="card bg-neutral p-4 w-2/5 m-auto ">
    <div class="text-3xl mb-2 text-neutral-content">Отгрузка на склад</div>
    ...
    <div class="form-control w-full">
      ...
      <input type="text" class="input input-bordered w-full" readonly v-model="shipmentContent.amount">
    </div>

    <button class="btn btn-primary mt-4" @click="this.$router.push(`/shipment_content/${shipmentContent.shipmentContentId}/edit`)">Редактировать</button>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      shipmentContent: {
        shipmenContentId: this.$route.params.id,
      },
      productsInWarehouse: [],
      shipments: [],
      multiselectClasses
  ...
  created() {
    ...
    axios.get(`/ShipmentContent/${this.shipmentContent.shipmenContentId}`).then((response) => this.shipmentContent = response.data)
  }
}
</script>